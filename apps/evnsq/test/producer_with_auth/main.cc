...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, optstring, long_options, &option_index)) != -1) {
    switch (opt) {
      case 't':nsqd_tcp_addr = optarg;
        break;

      case 'h':lookupd_http_url = optarg;
        break;

      case 's':auth_secret = optarg;
        break;

      case 'c':total_count = size_t(std::atoi(optarg));
        break;

      default:printf("error argument [%s]\n", argv[optind]);
        return -1;
    }
  }
