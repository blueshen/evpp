...
NSQConn::NSQConn(Client *c, const Option &ops)
    : nsq_client_(c), loop_(c->loop()), option_(ops), status_(kDisconnected)
    //, wait_ack_count_(0)
    , published_count_(0), published_ok_count_(0), published_failed_count_(0) {
  DLOG_TRACE;
}
...
void NSQConn::Reconnect() {
  LOG_WARN << "NSQConn::Close() this=" << this << " status=" << StatusToString() << " remote_nsq_addr="
           << remote_addr();
...
void NSQConn::OnRecv(const evpp::TCPConnPtr &conn, evpp::Buffer *buf) {
  while (buf->size() > 4) {
    ...
    switch (status_) {
      case evnsq::NSQConn::kDisconnected:break;

      case evnsq::NSQConn::kConnecting:break;
      ...
      case evnsq::NSQConn::kConnected:assert(false && "It should never come here.");
        break;
      ...
      case evnsq::NSQConn::kReady:OnMessage(body_len, frame_type, buf);
        break;
...
void NSQConn::OnMessage(size_t message_len, int32_t frame_type, evpp::Buffer *buf) {
  ...
  switch (frame_type) {
    case kFrameTypeResponse:LOG_INFO << "frame_type=" << frame_type << " kFrameTypeResponse. ["
                                     << std::string(buf->data(), message_len) << "]";
...
void NSQConn::OnPublishResponse(const char *d, size_t len) {
  ...
  if (len == 2 && d[0] == 'O' && d[1] == 'K') {
    published_ok_count_++;
    LOG_INFO << "Get a PublishResponse message 'OK', command=" << cmd.get() << " published_ok_count="
             << published_ok_count_;
