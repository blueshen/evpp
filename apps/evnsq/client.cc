...
void Client::ConnectToLookupd(const std::string &lookupd_url/*http://127.0.0.1:4161/lookup?topic=test*/) {
  auto f = [this, lookupd_url]() {
    ...
    std::shared_ptr<evpp::httpc::Request>
        r(new evpp::httpc::Request(this->loop_, lookupd_url, "", evpp::Duration(1.0)));
...
void Client::OnConnection(const NSQConnPtr &conn) {
  ...
  switch (conn->status()) {
    case NSQConn::kConnecting:MoveToConnectingList(conn);
      break;
    case NSQConn::kConnected:
      ...
      break;
    case NSQConn::kReady:assert(type_ == kConsumer);
      conns_.push_back(conn);
      connecting_conns_.erase(conn->remote_addr());
      break;
    default:
