...
void SetCommand::RequestBuffer(std::string &buf) {
  ...
  buf.resize(
      sizeof(protocol_binary_request_header) + sizeof(uint32_t) + sizeof(uint32_t) + key_.size() + value_.size());
  int index = 0;
...
void PrefixMultiGetCommand::PacketRequest(const std::vector<std::string> &keys,
