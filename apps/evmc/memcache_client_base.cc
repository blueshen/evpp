...
void MemcacheClientBase::BuilderMemClient(evpp::EventLoop *loop,
                                          std::string &server,
...
MemcacheClientBase::MemcacheClientBase(const char *vbucket_conf)
    : vbucket_conf_(vbucket_conf),
      load_loop_(nullptr),
      load_thread_(nullptr),
      vbconf_cur_(nullptr),
      vbconf_1_(new MultiModeVbucketConfig()),
      vbconf_2_(new MultiModeVbucketConfig()) {
  assert(vbconf_1_);
  assert(vbconf_2_);
}
