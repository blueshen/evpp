...
void MemcacheClientPool::Set(evpp::EventLoop *caller_loop,
...
void MemcacheClientPool::MultiGet(evpp::EventLoop *caller_loop,
...
void MemcacheClientPool::PrefixMultiGet(evpp::EventLoop *caller_loop,
                                        std::vector<std::string> &keys,
  ...
  PrefixMultiKeyGetHandlerPtr
      handler = std::make_shared<MultiKeyHandler<PrefixMultiGetResult, PrefixMultiGetCallback> >(callback);
  auto &result = handler->get_result();
  std::size_t pos = 0;
