...
void MemcacheClient::PushRunningCommand(CommandPtr &cmd) {
  ...
  if (UNLIKELY(!timeout_.IsZero() && timer_canceled_)) {
    cmd_timer_ =
        exec_loop_->RunAfter(timeout_, std::bind(&MemcacheClient::OnPacketTimeout, shared_from_this(), cmd->id()));
...
void MemcacheClient::PushWaitingCommand(CommandPtr &cmd) {
  ...
  if (UNLIKELY(!timeout_.IsZero() && con_timer_canceled_)) {
    con_cmd_timer_ =
        exec_loop_->RunAfter(timeout_, std::bind(&MemcacheClient::OnConnectTimeout, shared_from_this(), cmd->id()));
