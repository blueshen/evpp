...
namespace evpp {
namespace evpphttp {
Service::Service(const std::string &listen_addr, const std::string &name, uint32_t thread_num) : listen_addr_(
    listen_addr), name_(name), thread_num_(thread_num) {
...
int Service::RequestHandler(const evpp::TCPConnPtr &conn, evpp::Buffer *buf, HttpRequest &hr) {
  ...
  if (expect != hr.field_value.end() && !hr.is_send_continue()
      && evutil_ascii_strcasecmp(expect->first.data(), "100-continue")) {
    HttpResponse resp(hr);
