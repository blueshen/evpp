...
bool Server::Init(int listen_port) {
  ...
  if (!lt.hservice->Listen(listen_port)) {
    int serrno = errno;
    LOG_ERROR << "this=" << this << " http server listen at port " << listen_port << " failed. errno=" << serrno << " "
              << strerror(serrno);
    lt.hservice->Stop();
    return false;
  }
...
void Server::Dispatch(EventLoop *listening_loop,
  ...
  DLOG_TRACE << "dispatch request " << ctx->req() << " url=" << ctx->original_uri() << " in main thread. status="
             << StatusToString();
  if (!IsRunning()) {
