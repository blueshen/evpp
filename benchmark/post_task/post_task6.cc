...
class PostTask {
 public:
  PostTask(int thread_count, uint64_t post_count)
      : thread_count_(thread_count), total_post_count_(post_count), pool_(NULL, thread_count) {
  }
...
int main(int argc, char *argv[]) {
  ...
  p.Start();
  p.Wait();
  LOG_WARN << argv[0] << " thread_count=" << thread_count << " post_count=" << post_count << " use time: "
           << p.use_time() << " seconds\n";
  return 0;
}
