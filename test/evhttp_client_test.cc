...
TEST_UNIT(testHTTPRequest2) {
  ...
  t.Start(true);
  evpp::httpc::Request
      *r = new evpp::httpc::Request(t.loop(), "http://www.360.cn/robots.txt?a=1", "", evpp::Duration(2.0));
  LOG_INFO << "Do http request";
  r->Execute(std::bind(&HandleHTTPResponse, std::placeholders::_1, &t));
...
TEST_UNIT(testHTTPRequest4) {
  ...
  evpp::EventLoopThread t;
  t.Start(true);
  evpp::httpc::PostRequest
      *r = new evpp::httpc::PostRequest(t.loop(), "http://www.360.cn/robots.txt?a=1", "", evpp::Duration(2.0));
  LOG_INFO << "Do http request";
  r->Execute(std::bind(&HandleHTTPResponse, std::placeholders::_1, &t));
...
static void HandleHTTPResponse(const std::shared_ptr<evpp::httpc::Response> &r,
