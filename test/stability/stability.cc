...
static void RequestHandler(evpp::EventLoop *loop,
                           const evpp::http::ContextPtr &ctx,
                           const evpp::http::HTTPSendResponseCallback &cb) {
  LOG_INFO << "DefaultRequestHandler loop=" << loop << " ctx.url=" << ctx->original_uri() << " tid="
...
static void DefaultRequestHandler(evpp::EventLoop *loop,
                                  const evpp::http::ContextPtr &ctx,
                                  const evpp::http::HTTPSendResponseCallback &cb) {
  LOG_INFO << "DefaultRequestHandler loop=" << loop << " ctx.url=" << ctx->original_uri() << " tid="
           << std::this_thread::get_id();
...
int main(int argc, char *argv[]) {
  ...
  for (size_t i = 0;; i++) {
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPServer1 "
        << i;
    TestTCPServer1();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestHTTPServer "
        << i;
    TestHTTPServer();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientReconnect "
        << i;
    TestTCPClientReconnect();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientConnectFailed "
        << i;
    TestTCPClientConnectFailed();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientDisconnectImmediately "
        << i;
    TestTCPClientDisconnectImmediately();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientDisconnectAndDestruct "
        << i;
    TestTCPClientDisconnectAndDestruct();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPClientConnectLocalhost "
        << i;
    TestTCPClientConnectLocalhost();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPServerSilenceShutdown1 "
        << i;
    TestTCPServerSilenceShutdown1();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestTCPServerSilenceShutdown2 "
        << i;
    TestTCPServerSilenceShutdown2();
    LOG_WARN
        << "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx Running test loop TestDNSResolver "
        << i;
    TestDNSResolver();
  }
  return 0;
